...
public class ViewAdapter extends RecyclerView.Adapter<ViewAdapter.ViewHolder> {
    ...
    private static final String TAG = "jerryzhuH";
    ...
    class SimpleUrlRequestCallback extends UrlRequest.Callback {
        ...
        public void onRedirectReceived(
            ...
            android.util.Log.i(TAG, "****** HTTP onRedirectReceived ******");
            request.followRedirect();
        }

        @Override
        public void onResponseStarted(UrlRequest request, UrlResponseInfo info) {
            android.util.Log.i(TAG, "******HTTP Response Started ******");
            android.util.Log.i(TAG, "***HTTP Headers Are *** " + info.getAllHeaders());
        ...
        public void onReadCompleted(
            ...
            android.util.Log.i(TAG, "******HTTP 读取中 ******" + byteBuffer);
            byteBuffer.flip();
            try {
                receiveChannel.write(byteBuffer);
            } catch (IOException e) {
                android.util.Log.i(TAG, "IO异常 HTTP IOException during ByteBuffer read. Details: ", e);
            }
            byteBuffer.clear();
        ...
        public void onSucceeded(UrlRequest request, UrlResponseInfo info) {
            ...
            android.util.Log.i(TAG,
                    "******请求完成 HTTP : , 状态码 " + info.getHttpStatusCode()
                            + ", 总接收字节数为  " + info.getReceivedByteCount() + "   KB为: " + info.getReceivedByteCount() / 1024);
            // Set the latency
            ((MainActivity) context).addCronetLatency(l, 1);
        ...
        public void onFailed(UrlRequest var1, UrlResponseInfo var2, CronetException var3) {
            android.util.Log.i(TAG, "****** 失败 HTTP , 错误信息:  " + var3.getMessage());
        }
    }
}
